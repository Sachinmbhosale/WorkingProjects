 ...
 *
 * Created
 * 28/02/2015   Sabina              DMS5-3485              Control rendering in formtype dataentry page
 * 20/03/2015   Sabina              DMS5-3579              Bringing a new checkbox to denote form type dataentry requires with image or without image  
 * 23/03/2015   Sabina              DMS5-3581              Binding controls and images in data entry page based on page number the control belongs to
 * 07 Apr2015                       DMS5-3854              Binding image modified in form dataentry after code review
 ...
 * 09/04/2015   Sharath             DMS5-3424              Auto Calculate Age on Current Date Selection
 * 09/04/2015   Sharath             DMS5-3425              Dual Data Entry Functionality
 * 10/04/2015   Sharath             DMS5-3473              Calculate year diffrence between user input and refer date field
 * 10/04/2015   Sharath             DMS5-3474              Formula Calculation
 * 11/04/2015   Sharath             DMS5-3627              Document/Image Uplaod
 ...
 * 15-05-2015   Gokul               DMS5-4193              Setting font size in form data entry
 * 03-6-2015    Gokul               DMS5-4330              Scanned document when uploaded for Data entry overwrites the template
 ...
 * 14 Jul  2015 Sharath             DMS5-4662              The form data entry is disrupted due to the implementation of "Info field"
 ...
 * 31/07/2015   Sharath             DMSENH6-4731	       Form Based Data Entry viewer span across entire page
...
{
    public partial class ManageFormDataEntry : PageBase
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                sitePath.Text = "WorkFlow Tasks >> Task Items >> Form Type Data Entry";
            ...
            ScriptManager.RegisterStartupScript(this, typeof(Page), "setfocus", "setFocus();", true);

            if (hdnSaveStatus.Value != "Cancel")
            {
                strImagePath = BindControls();// DMS5-3485                
        ...
        protected void CheckFinalstatus()
        {
            ...
            if (Objresult.dsResult.Tables[0].Rows.Count > 0)
            {
                ...
                {
                    PanelFormControls.Enabled = false;
        ...
        /// <summary>
        /// Get all the stage related data
        /// </summary>
        protected void GetStageDetails()
        {
            DBResult Objresult = new DBResult();

            //To check background image is required or not for form typedataentry
            objStage = GetValuesFromSession();
            Objresult = objStage.ManageWorkflowStages(objStage, "GetStageDetails");
            Logger.Trace("Get all stage related data from database", Session["LoggedUserId"].ToString());
            if (Objresult.dsResult != null && Objresult.dsResult.Tables.Count > 0 && Objresult.dsResult.Tables[0].Rows.Count > 0)
            {
                //get the form type with background image or without background image
                hdnShowBackgroundImage.Value = Convert.ToString(Objresult.dsResult.Tables[0].Rows[0]["Workflowstage_bShowBackgroundImage"]);
            }
        }
        ...
        void ddlFilter_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if ((hdnCurrentDDL.Value == "") || (ddlParent.ID.ToString() == hdnCurrentDDL.Value))
            {
                ...
                if (Objresult.dsResult != null && Objresult.dsResult.Tables.Count > 0 && Objresult.dsResult.Tables[1] != null && Objresult.dsResult.Tables[1].Rows.Count > 0)
                {
                    foreach (DataRow dr in Objresult.dsResult.Tables[1].Rows)
                    {
                        ...
                        {
                            Logger.Trace("Error in dropdownlist value binding", Session["LoggedUserId"].ToString());
                        }
                    }
            ...
            {
                InitializeImages(strImagePath);
            }
        }

        // DMS5-3485  binding styles
        public Control BindStylesToControl(Control ControlName, bool withIamgeDataEntry)
        {
            int Height = Convert.ToInt32(hdnHeight.Value);  /* DMS5-4193 M */
            /* DMS5-4193 M */


            if (ControlName.GetType() == typeof(DropDownList))
            {
                DropDownList ddl = (DropDownList)ControlName;
                //if (withIamgeDataEntry.Equals(false))
                //{
                //    ddl.Style.Add("left", Convert.ToInt32(hdnX1.Value) * 2 + "px");
                //    ddl.Style.Add("top", (Convert.ToInt32(hdnY1.Value) * 2) + 25 + "px");
                //}
                //else
                //{
                ddl.Style.Add("left", hdnX1.Value + "px");
                ddl.Style.Add("top", hdnY1.Value + "px");
                //}

                ddl.Style.Add("Width", hdnWidth.Value + "px");
                ddl.Style.Add("Height", Height + "px");
                //ddl.Style.Add("Height", hdnHeight.Value + "px");
                ddl.Style.Add("position", "absolute");
                ddl.Style.Add("padding", "0px");
            }
            else if (ControlName.GetType() == typeof(Panel))
            {
                Panel pnl = (Panel)ControlName;
                pnl.Style.Add("left", hdnX1.Value + "px");
                pnl.Style.Add("top", hdnY1.Value + "px");
                pnl.Style.Add("Width", hdnWidth.Value + "px");
                pnl.Style.Add("Height", hdnHeight.Value + "px");
                pnl.Style.Add("position", "absolute");
                pnl.Style.Add("padding", "0px");
            }
            else if (ControlName.GetType() == typeof(CheckBoxList))
            {
                CheckBoxList chk = (CheckBoxList)ControlName;
                //if (withIamgeDataEntry.Equals(false))
                //{
                //    chk.Style.Add("left", Convert.ToInt32(hdnX1.Value) * 2 + "px");
                //    chk.Style.Add("top", (Convert.ToInt32(hdnY1.Value) * 2) + 25 + "px");
                //}
                //else
                //{
                chk.Style.Add("left", hdnX1.Value + "px");
                chk.Style.Add("top", hdnY1.Value + "px");
                //}

                chk.Style.Add("Width", hdnWidth.Value + "px");
                chk.Style.Add("Height", hdnHeight.Value + "px");
                chk.Style.Add("position", "absolute");
                chk.Style.Add("padding", "0px");
            }
            else if (ControlName.GetType() == typeof(TextBox))
            {
                TextBox txt = (TextBox)ControlName;
                //if (withIamgeDataEntry.Equals(false))
                //{
                //    txt.Style.Add("left", Convert.ToInt32(hdnX1.Value) * 2 + "px");
                //    txt.Style.Add("top", (Convert.ToInt32(hdnY1.Value) * 2 ) + 25 + "px");
                //}
                //else
                //{
                txt.Style.Add("left", hdnX1.Value + "px");
                txt.Style.Add("top", hdnY1.Value + "px");

                //}
                txt.Style.Add("Width", hdnWidth.Value + "px");
                //txt.Style.Add("font-size", Fontsize + "px");
                txt.Style.Add("Height", Height + "px");
                txt.Style.Add("position", "absolute");
                txt.Style.Add("padding", "0px");
                /* DMS5-4193 BS */
                if (Height <= 10)
                {
                    txt.Font.Size = 6;
            ...
            else if (ControlName.GetType() == typeof(AsyncFileUpload))
            {
                AsyncFileUpload asyncFileUpload = (AsyncFileUpload)ControlName;
                //if (withIamgeDataEntry.Equals(false))
                //{
                //    asyncFileUpload.Style.Add("left", Convert.ToInt32(hdnX1.Value) * 2 + "px");
                //    asyncFileUpload.Style.Add("top", (Convert.ToInt32(hdnY1.Value) * 2) + 25 + "px");
                //}
                //else
                //{
                asyncFileUpload.Style.Add("left", hdnX1.Value + "px");
                asyncFileUpload.Style.Add("top", hdnY1.Value + "px");
                //}              
                asyncFileUpload.Style.Add("Width", hdnWidth.Value + "px");
                asyncFileUpload.Style.Add("Height", hdnHeight.Value + "px");
                asyncFileUpload.Style.Add("position", "absolute");
                asyncFileUpload.Style.Add("padding", "0px");
            }
            else if (ControlName.GetType() == typeof(Label))
            {
                Label label = (Label)ControlName;
                //if (withIamgeDataEntry.Equals(false))
                //{
                //    label.Style.Add("left", Convert.ToInt32(hdnX1.Value) * 2 + "px");
                //    label.Style.Add("top", (Convert.ToInt32(hdnY1.Value) * 2) + 25 + "px");
                //}
                //else
                //{
                label.Style.Add("left", hdnX1.Value + "px");
                label.Style.Add("top", hdnY1.Value + "px");
                //}  

                label.Style.Add("Width", hdnWidth.Value + "px");
                label.Style.Add("Height", hdnHeight.Value + "px");
                label.Style.Add("position", "absolute");
                label.Style.Add("padding", "0px");
            }
            else if (ControlName.GetType() == typeof(ImageButton))
            {
                ImageButton imageButton = (ImageButton)ControlName;
                //if (withIamgeDataEntry.Equals(false))
                //{

                //    imageButton.Style.Add("top", (Convert.ToInt32(hdnY1.Value) * 2) + 25 + "px");
                //}
                //else
                //{
                imageButton.Style.Add("top", hdnY1.Value + "px");
                //}  

                // imageButton.Style.Add("left", hdnX1.Value + "px");
                imageButton.Style.Add("top", hdnY1.Value + "px");
                imageButton.Style.Add("Width", hdnWidth.Value + "px");
                imageButton.Style.Add("Height", hdnHeight.Value + "px");
                imageButton.Style.Add("position", "absolute");
                imageButton.Style.Add("padding", "0px");
            }
        ...
        //DMS5-3854BS
        private bool InitializeImages(string strFilePath)
        {
            string originalImagesFolder = string.Empty;
            string fileExtension = string.Empty;
            int pageCount = 0;
            string imageUrl = string.Empty;
            string imageFolderPath = Server.MapPath(@"~");
            imageFolderPath = imageFolderPath + GetImageVirtualFolderPath();
            bool withIamgeDataEntry = false; // DMS5-3904 A

            if (strFilePath.Length > 0)
            {
                originalImagesFolder = beforedot(strFilePath);

                fileExtension = ".pdf"; //document splitted files always will be in pdf format
                pageCount = System.IO.Directory.GetFiles(originalImagesFolder, "*" + fileExtension).Length;
            }

            if (pageCount > 0)
            {
                Logger.Trace("Pages count " + pageCount, Session["LoggedUserId"].ToString());

                if (!IsPostBack)
                    ConvertPdfToJpg(originalImagesFolder, imageFolderPath, pageCount);
                Logger.Trace("Converted pdf to jpg", Session["LoggedUserId"].ToString());

                //DMS5-3904BS -- Shifted lines inside this if block 
                //Using handler path to bind images source
                imageUrl = GetSrc("Handler") + imageFolderPath + @"\";
                //DMS5-3904BE
                BindDocumentImagePanel(imageUrl, pageCount); // DMS5-3904 M

                // DMS5-3904 A
                withIamgeDataEntry = true; // this value is used when binding data entry image panels
            }

            //Load images for DataEntry section
            //DMS5-3904BS  -- Re-written code to Load background image from template folder
            // Data entry without image. Load background image from template folder
            fileExtension = ".jpg"; // DMS5-3904 A //template splitted files always will be in jpg format
            originalImagesFolder = GetTemplateOriginalFolderPath();
            pageCount = System.IO.Directory.GetFiles(originalImagesFolder, "*" + fileExtension).Length;
            ...
            if (pageCount > 0)
            {
                ...
                //Using handler path to bind images source
                imageUrl = GetSrc("Handler") + originalImagesFolder + @"\";
                withIamgeDataEntry = BindDataEntryImagePanel(imageUrl, pageCount, withIamgeDataEntry); // DMS5-3904 M
            }
            // DMS5-3904BE
            return withIamgeDataEntry;
        }
        ...
        // DMS5-4330 BS-- Block modified
        /// <summary>
        /// Render images - from document path
        /// </summary>
        public void BindDocumentImagePanel(string imagesVirtualPath, int pagesCount)
        {
            Logger.Trace("Binding image to panel started", Session["LoggedUserId"].ToString());
            for (int i = 1; i <= pagesCount; i++)
            {
                string imagePath = imagesVirtualPath + "/" + i + ".jpg";
                Logger.Trace("Image url:" + imagePath, Session["LoggedUserId"].ToString());

                //Document display panel
                Panel pnl = new Panel();
                pnl.Attributes.Add("style", "display:block");
                pnl.ID = "pnl_" + i;
                pnl.Attributes.Add("style", "width:600px");
                pnl.Attributes.Add("style", "height:400px");
                pnl.Attributes.Add("style", "overflow:auto");
                pnl.Attributes.Add("style", "position:relative");
                Image imgPnl = new Image();
                imgPnl.ID = imgPnl + "imgPnl" + i.ToString();
                imgPnl.Attributes.Add("style", "margin-bottom:3px");
                imgPnl.ImageUrl = imagePath;
                pnl.Controls.Add(imgPnl);
                divcropbox.Controls.Add(pnl);
                lblOriginalImage.Visible = true;
            }
        }
        // DMS5-4330 BE
        ...
        // DMS5-4330 BS
        /// <summary>
        /// Render images - from template
        /// </summary>
        /// <param name="imagesVirtualPath"></param>
        /// <param name="pagesCount"></param>
        /// <param name="withIamgeDataEntry"></param>
        public bool BindDataEntryImagePanel(string imagesVirtualPath, int pagesCount, bool withIamgeDataEntry = false)
        {
            bool showBackgroundImage = hdnShowBackgroundImage.Value.Length > 1 ? Convert.ToBoolean(hdnShowBackgroundImage.Value.ToLower()) : Convert.ToBoolean(Convert.ToInt32(hdnShowBackgroundImage.Value));
            Logger.Trace("Binding image to panel started", Session["LoggedUserId"].ToString());
            for (int i = 1; i <= pagesCount; i++)
            {
                string imagePath = imagesVirtualPath + "/" + i + ".jpg";
                ...
                // Data Entry images panel
                Panel pnlDataEntry = new Panel();
                pnlDataEntry.Attributes.Add("style", "display:block");
                pnlDataEntry.ID = "ControlPnl" + i;
                pnlDataEntry.Attributes.Add("style", "height:400px");
                pnlDataEntry.Attributes.Add("style", "overflow:auto");
                pnlDataEntry.Attributes.Add("style", "position:relative");
                Image img = new Image();
                img.ID = img + i.ToString();
                img.Attributes.Add("style", "margin-bottom:3px");
                img.ImageUrl = imagePath;

                //DMSENH6-4731 BS
                if (!withIamgeDataEntry)
                {
                    divcropbox.Attributes.Add("style", "display:none;");
                    lblOriginalImage.Visible = false;
                    tblMessageImg.Style.Add("display", "none;");
                    tblDetails.Style.Add("margin-left", "auto;");
                    tblDetails.Style.Add("margin-right", "auto;");
                    tdTable_PrevStage.Style.Add("display", "none;");
                    tblImageSection.Style.Add("margin-left", "auto;");
                    tblImageSection.Style.Add("margin-right", "auto;");
                }

                ControlPlaceHolder_CurStage.Attributes.Add("width", "638px;");
                pnlDataEntry.Attributes.Add("style", "width:600px");
                ...
                pnlDataEntry.Controls.Add(img);
                ControlPlaceHolder_CurStage.Controls.Add(pnlDataEntry);
            }
        ...
        /// <summary>
        /// Get stagefields values from session
        /// </summary>
        /// <returns></returns>
        public WorkflowStageField GetStageFieldValues()
        {
            ...
            {
                objStageField.WorkflowStageFieldsStageId = 0;
            }
            if (Session["FieldId"] != null && Convert.ToString(Session["FieldId"]) != string.Empty)
            {
        ...
        protected string GetImageVirtualFolderPath()
        {
            ...
            Logger.Trace("Virtual folder path of data entry images" + path, Session["LoggedUserId"].ToString());
            return path;
        }
        //DMS5-3854BE
        ...
        private string GenerateFieldSaveQuery()
        {
            ...
            BuildQuery(dtPrevFields, ControlPlaceHolder_CurStage);
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                //check if any error message is dispalyed
        ...
        protected string BindControls()
        {
            ...
            try
            {
                ...
                if (Objresult.dsResult != null && Objresult.dsResult.Tables.Count > 0 && Objresult.dsResult.Tables[3].Rows.Count > 0)
                {
                    ...
                    withIamgeDataEntry = InitializeImages(strImagePathLocal);
                    if (strImagePathLocal.Length > 0)
                    {
                        lblMessageImg.Text = "";
                    }
                    else
                    {
                        lblMessageImg.Text = string.Empty;//DMSENH6-4731
                        Logger.Trace("No Image avaiable", Session["LoggedUserId"].ToString());
                    }
                }
                ...
                {
                    RenderControls(Objresult.dsResult.Tables[1], Objresult.dsResult.Tables[3], ControlPlaceHolder_CurStage, withIamgeDataEntry);
                ...
                if (Objresult.dsResult != null && Objresult.dsResult.Tables.Count > 0 && Objresult.dsResult.Tables[2].Rows.Count > 0)
                {
                    ...
                    RenderControls(Objresult.dsResult.Tables[2], Objresult.dsResult.Tables[3], ControlPlaceHolder_CurStage, withIamgeDataEntry);
                }
                ...
                if (Objresult.dsResult != null && Objresult.dsResult.Tables.Count > 0 && Objresult.dsResult.Tables[4].Rows.Count > 0)
                {
                    ...
                    RenderControls(Objresult.dsResult.Tables[4], Objresult.dsResult.Tables[3], ControlPlaceHolder_CurStage, withIamgeDataEntry);//DMS5-4662
                }
        ...
        protected void RenderControls(DataTable controlsConfig, DataTable controlsData, System.Web.UI.HtmlControls.HtmlGenericControl placeholder, bool withIamgeDataEntry)
        {
            bool showBackgroundImage = hdnShowBackgroundImage.Value.Length > 1 ? Convert.ToBoolean(hdnShowBackgroundImage.Value.ToLower()) : Convert.ToBoolean(Convert.ToInt32(hdnShowBackgroundImage.Value));
            string labelWidth = string.Empty;
            int labelLeftPosition = 0;
            int pageNo = 0;
            Int16 lastSortOrder = hdnlastSortOrder.Value.Length > 0 ? Convert.ToInt16(hdnlastSortOrder.Value) : Convert.ToInt16(0);
            ...
            try
            {
                ...
                if (controlsConfig.Rows.Count > 0)
                {
                    ...
                    {
                        pageNo = Convert.ToInt32(dr["WorkflowStageFields_iPageNo"]);
                        //Common to bind label
                        hdnX1.Value = Convert.ToString(dr["WorkflowStageFields_iLocationX1"]);
                        hdnX2.Value = Convert.ToString(dr["WorkflowStageFields_iLocationX2"]);
                        hdnY1.Value = Convert.ToString(dr["WorkflowStageFields_iLocationY1"]);
                        hdnY2.Value = Convert.ToString(dr["WorkflowStageFields_iLocationY2"]);
                        hdnWidth.Value = Convert.ToString(dr["WorkflowStageFields_iWidth"]);
                        hdnHeight.Value = Convert.ToString(dr["WorkflowStageFields_iHeight"]);
                        labelWidth = "70";
                        labelLeftPosition = Convert.ToInt32(hdnX1.Value) - Convert.ToInt32(labelWidth) + 5;

                        Panel Paneldiv = (Panel)ControlPlaceHolder_CurStage.FindControl("ControlPnl" + pageNo);
                        if (Paneldiv != null)
                        {
                            Paneldiv.Attributes.Add("position", "relative;");

                            if (showBackgroundImage == false)//checking whether form type is with background image or not
                            {
                                Label lbl = new Label();
                                lbl.ID = dr["WorkflowStageFields_ControlID"].ToString() + "_Label";
                                lbl.Text = dr["WorkflowStageFields_vLabelText"].ToString();
                                lbl.Style.Add("top", hdnY1.Value + "px");
                                lbl.Style.Add("Width", labelWidth + "px");
                                lbl.Style.Add("Height", hdnHeight.Value + "px");
                                lbl.Style.Add("left", labelLeftPosition + "px");
                                lbl.Style.Add("Border", "0px");
                                lbl.Style.Add("Padding", "0px");
                                lbl.Style.Add("position", "absolute");
                                //DMS5-4193 BS
                                if (Convert.ToInt32(hdnHeight.Value) <= 10)
                                {
                                    lbl.Font.Size = 6;
                                }
                                //DMS5-4193 BE
                                Paneldiv.Controls.Add(lbl);
                            }
                        }
                        ...
                        Int16 sortOrder = Convert.ToInt16(dr["WorkflowStageFields_iSortOrder"]);
                        sortOrder = Convert.ToInt16(sortOrder + lastSortOrder);
                        ...
                        if (objType.Equals(Convert.ToString(FieldTypes.DropDown)))
                        {
                            ...
                            ddl = (DropDownList)BindStylesToControl(ddl, withIamgeDataEntry);//bind the styles to controls to position control based on coordinate value
                            ddl.TabIndex = sortOrder;
                            Paneldiv.Controls.Add(ddl);
                        ...
                        else if (objType.Equals(Convert.ToString(FieldTypes.MultiDropDown)))
                        {
                            ...
                            pnl = (Panel)BindStylesToControl(pnl, withIamgeDataEntry);//bind the styles to controls to position control based on coordinate value
                            Paneldiv.Controls.Add(pnl);
                        ...
                        else if (objType.Equals(Convert.ToString(FieldTypes.TextBox)))
                        {
                            ...
                            textbox = (TextBox)BindStylesToControl(textbox, withIamgeDataEntry);//bind the styles to controls to position control based on coordinate value
                            textbox.TabIndex = sortOrder;
                            Paneldiv.Controls.Add(textbox);
                            ...
                            else if (dbType.Equals("Date"))
                            {
                                ...
                                Paneldiv.Controls.Add(imgButton);
                                ...
                                Paneldiv.Controls.Add(Cal);

                                #region Set position for date controls
                                textbox.Style.Add("left", hdnX1.Value + "px");
                                textbox.Style.Add("top", hdnY1.Value + "px");
                                // reduce width; calendar extender width
                                // hdnWidth.Value = Convert.ToString(Convert.ToInt32(hdnWidth.Value) - 20);
                                //textbox.Style.Add("Width", hdnWidth.Value + "px");
                                // textbox.Style.Add("Height", hdnHeight.Value + "px");
                                textbox.Style.Add("position", "absolute");

                                //cal icon; icon width is 15px and add 
                                imgButton.Style.Add("left", Convert.ToInt32(hdnX1.Value) + Convert.ToInt32(hdnWidth.Value) + 4 + "px");
                                imgButton.Style.Add("top", hdnY1.Value + "px");
                                imgButton.Style.Add("position", "absolute");
                                #endregion
                            }
                        }
                        #endregion
                            ...
                            || objType.Equals(Convert.ToString(FieldTypes.DateCalculation_ReferDate)))
                        {
                            ...
                            try
                            {
                                ...
                                calExt.ID = "Cal_" + textbox.ID;
                                calExt.Format = "dd/MM/yyyy";
                                calExt.TargetControlID = textbox.ID;
                                calExt.PopupButtonID = imgButton.ID;
                                Paneldiv.Controls.Add(calExt);

                                //Label to Display calculated date diffrence
                                Label lblDateDiffrence = new Label();
                                lblDateDiffrence.ID = controlID + "_LblCalculateAge";
                                Paneldiv.Controls.Add(lblDateDiffrence);
                                ...
                                //cal icon; icon width is 15px and add 
                                lblDateDiffrence.Style.Add("left", hdnX1.Value + "px");
                                lblDateDiffrence.Style.Add("top", Convert.ToInt32(hdnY1.Value) + Convert.ToInt32(hdnHeight.Value) + 4 + "px");
                                lblDateDiffrence.Style.Add("position", "absolute");
                                #endregion
                                ...
                                else
                                {
                                    ...
                                    if (matchingControlId.Length != 0)
                                    {
                                        ...
                                        TextBox txtReferredField = FindControlRecursive(Paneldiv, referredControlId) as TextBox;
                            ...
                            || objType.Equals(Convert.ToString(FieldTypes.DualDatEntry_MaskBoth)))
                        {
                            try
                            {
                                ...
                                textbox.Attributes.Add("onchange", "return ValidateDataMismatch('" + textbox.ID + "','" + txtMask.ID + "');");
                                txtMask.Attributes.Add("onchange", "return ValidateDataMismatch('" + textbox.ID + "','" + txtMask.ID + "');");
                                txtMask.ToolTip = "Retype " + labelText;

                                txtMask = (TextBox)BindStylesToControl(txtMask, withIamgeDataEntry);//bind the styles to controls to position control based on coordinate value
                                txtMask.TabIndex = sortOrder;
                                Paneldiv.Controls.Add(txtMask);
                        ...
                        else if (objType.Equals(Convert.ToString(FieldTypes.CalculateField)))
                        {
                            try
                            {
                                TextBox textbox = new TextBox();
                                textbox.ID = dr["WorkflowStageFields_ControlID"].ToString();
                                textbox.ToolTip = dr["WorkflowStageFields_vFormula"].ToString();
                                textbox.Text = fieldValue;
                                ...
                                foreach (string controlId in controlsIdList)
                                {
                                    ...
                                    string find = "ReferredFieldId = '" + referredControlId + "' AND  FormulaFieldId = '" + textbox.ID + "'";
                                    DataRow[] foundRows = FormulaFieldData.Select(find);
                                    if (foundRows.Length.Equals(0))
                                    {
                                        DataTable dt = FormulaFieldData;
                                        dt.Rows.Add(referredControlId, referredFieldName, referedFieldIds, textbox.ID, textbox.ToolTip);
                                        FormulaFieldData = dt;
                                    }
                                }
                                #endregion
                        ...
                        else if (objType.Equals(Convert.ToString(FieldTypes.Document)))
                        {
                            try
                            {
                                ...
                                asyncFileUpload.UploadedComplete += new EventHandler<AsyncFileUploadEventArgs>(AsyncFileUpload1_UploadedComplete);
                                asyncFileUpload.ID = controlID;
                                asyncFileUpload.EnableViewState = false;
                                asyncFileUpload.ToolTip = fieldValue;
                                asyncFileUpload.TabIndex = sortOrder;
                                Paneldiv.Controls.Add(asyncFileUpload);

                                //Disable controls if fieldEditable is set to false
                                //DMS5-4331
                                asyncFileUpload.Enabled = fieldEditable;
                                ...
                                asyncFileUpload.ThrobberID = lblThrobber.ID;
                                Paneldiv.Controls.Add(lblThrobber);
                                ...
                                Paneldiv.Controls.Add(lnkViewFile);
                                ...
                                // Set position for document controls
                                //async file upload
                                asyncFileUpload.Style.Add("z-index", "1");
                                asyncFileUpload.Style.Add("left", hdnX1.Value + "px");
                                asyncFileUpload.Style.Add("top", hdnY1.Value + "px");
                                // reduce width; view & delete button width
                                hdnWidth.Value = Convert.ToString(Convert.ToInt32(hdnWidth.Value) - 30);
                                asyncFileUpload.Width = Unit.Pixel(Convert.ToInt32(hdnWidth.Value)); // Style.Add("Width", hdnWidth.Value + "px");
                                asyncFileUpload.Style.Add("Height", hdnHeight.Value + "px");
                                asyncFileUpload.Style.Add("Padding", "0px");
                                asyncFileUpload.Style.Add("position", "absolute");
                                // view file; icon width is 15px and add 
                                lnkViewFile.Style.Add("left", Convert.ToInt32(hdnX1.Value) + Convert.ToInt32(hdnWidth.Value) + 5 + "px");
                                lnkViewFile.Style.Add("top", hdnY1.Value + "px");
                                lnkViewFile.Style.Add("Padding", "0px");
                                lnkViewFile.Style.Add("position", "absolute");

                                // delete file; 
                                //btnFileDelete.Style.Add("left", Convert.ToInt32(hdnX1.Value) + Convert.ToInt32(hdnWidth.Value) + 24 + "px");
                                //btnFileDelete.Style.Add("top", hdnY1.Value + "px");
                                //btnFileDelete.Style.Add("Padding", "0px");
                                //btnFileDelete.Style.Add("position", "absolute");
                        ...
                        if (mandatory.Equals(true))
                        {
                            ...
                            lblAstreick.Style.Add("top", hdnY1.Value + "px");
                            lblAstreick.Style.Add("left", labelLeftPosition + "px");
                            Paneldiv.Controls.Add(lblAstreick);
        ...
        protected string BuildQuery(DataTable controlsConfig, System.Web.UI.HtmlControls.HtmlGenericControl placeholder)
        {
