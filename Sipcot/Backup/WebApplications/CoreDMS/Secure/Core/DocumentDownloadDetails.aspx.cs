    ...
    public partial class DocumentDownloadDetails : PageBase
    {
        ...
        protected void btnSendMail_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                objDocumentMailBL.SendDocumentMail("Insert", 0, txtMailTo.Text, strSubject, strMailbody, strToken, strDownloadlink, strToken, orgId, out messagevalue, out ErrorState, out ErrorSeverity);
                if (ErrorState == 0)
                {
                    ...
                    {
                        if (File.Exists(hdnFileLocation.Value))
                            File.Copy(hdnFileLocation.Value, strDestinationfile + "\\" + hdnEncrpytFileName.Value, true);//Copy the file to new location
                    }
                    catch (Exception)
                    {
