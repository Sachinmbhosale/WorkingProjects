...
{
    public partial class WorkflowStudio :  PageBase
    {
        ...
        string unConfirmedProcess = "GetAllProcess";
        ...
        protected void BindOrganizationDropDown()
        {
            ...
            {
                lblMessageProcess.Text = ex.Message.ToString();
            }
        ...
        protected void btnProcessEdit_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (hdnAction.Value == "ProcessOnClick")
                {
        ...
        protected void btnProcessSave_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    Session["ProcessId"] = objProcess.WfProcessId = Convert.ToInt32(hiddenProcessId.Value);
                ...
                if (objResult.ErrorState == 0)
                {
                    ...
                    if (!strSaveStatus.Equals("Save Changes"))
                    {
                        ...
                        if (objResult.dsResult.Tables.Count > 0 && objResult.dsResult.Tables[0].Rows.Count > 0)
                        {
                            ...
                            foreach (DataRow row in objResult.dsResult.Tables[0].Rows)
                            {
                                 buttonId = Convert.ToInt32(row["ProcessID"]);
                    ...
                    //handling Workflow properties window
                   // BindWorkflowProjectDropDown();
                    BindSortOrderToListBox();

                    if (action.Equals("AddProcess"))
                    {

                        hdnSaveFlagStatus.Value = "Yes";
                        //make button color green on sucessful save
                        ScriptManager.RegisterClientScriptBlock((sender as Control), this.GetType(), "ChangeColor", "ChangeAddProcessButtonColor('" + hdnSaveFlagStatus.Value + "');", true);
                    }
                    tblProcessControls.Style.Add("display", "none");
                    tblProcessOwner.Style.Add("display", "block");
                ...
                else
                {
                    try
                    {
                        ...
                        tblProcessControls.Style.Add("display", "block");
                        tblProcessOwner.Style.Add("display", "none");
                    }
                    catch
                    {
                    }
            ...
            try
            {
                ...
                if (objResult.ErrorState == 0)
                {
                    ...
                    if (objResult.dsResult.Tables.Count > 0 && objResult.dsResult.Tables[0].Rows.Count > 0)
                    {
                        ...
                        foreach (DataRow row in objResult.dsResult.Tables[0].Rows)
                        {
                            buttonId = Convert.ToInt32(row["WorkflowId"]);
                            string buttonName = Convert.ToString(row["WorkflowName"]);
                            dictionary.Add(buttonId, buttonName);
                        }
                    ...
                    //Loading stages dataentry types
                    LoadDataEntryTypes();

                    if (action.Equals("AddWorkflow"))
                    {                        
                        hdnSaveFlagStatus.Value = "Yes";
                    }
                    //make button color green on sucessful save
                    //ScriptManager.RegisterClientScriptBlock((sender as Control), this.GetType(), "ChangeColor", "ChangeAddworkflowButtonColor('" + hdnSaveFlagStatus.Value + "');", true);
                    tblWorkflowControls.Style.Add("display", "none");
                    tblWorkflowOwner.Style.Add("display", "block");
                    btnWorkflowSave.Style.Add("display","none");
                    hdnErrorStatus.Value = "";
                }
                else
                {
                    btnWorkflowSave.Style.Add("display", "block");
                    tblWorkflowControls.Style.Add("display", "block");
                    tblWorkflowOwner.Style.Add("display", "none");
                    if (strSaveStatus == "Save Changes")
                    {
                        hdnErrorStatus.Value = "EDIT_ERROR";
                    }
        ...
        protected void btnWorkflowEdit_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                objStudio.LoginToken = loginUser.LoginToken;
                objStudio.LoginOrgId = loginUser.LoginOrgId;
                objStudio.ProcessId = Convert.ToInt32(hiddenProcessId.Value);
                ...
                if (objResult.dsResult.Tables.Count > 0 && objResult.dsResult.Tables[0].Rows.Count > 0)
                {
                    ...
                    if (dataRow["ProjectName"] != null && Convert.ToString(dataRow["ProjectName"]) != string.Empty)
                        ddlDmsProject.SelectedValue = Convert.ToString(dataRow["ProjectName"]);
        ...
        protected void btnStatusEdit_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                objStudio.LoginOrgId = loginUser.LoginOrgId;
                objStudio.ProcessId = Convert.ToInt32(hiddenProcessId.Value);
                objStudio.WorkflowId = Convert.ToInt32(hiddenWorkflowId.Value);
                objStudio.StageId = Convert.ToInt32(hiddenStageId.Value);
                objStudio.StatusId = Convert.ToInt32(hdnStatusId.Value);
                ...
                if (objResult.dsResult.Tables.Count > 0 && objResult.dsResult.Tables[0].Rows.Count > 0)
                {
                    ...
                    if (dataRow["Move To Stage"] != null && Convert.ToString(dataRow["Move To Stage"]) != string.Empty)
                        ddl_Stages.SelectedValue = Convert.ToString(dataRow["Move To Stage"]);
        ...
        protected void btnStatusOk_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                CommaSeparatedStatusNames = CommaSeparatedStatusNames.TrimEnd(',');
                CommaSeparatedStatusNames = Regex.Replace(CommaSeparatedStatusNames, "<.*?>", String.Empty);
                ScriptManager.RegisterClientScriptBlock((sender as Control), this.GetType(), "BuildAddStatus", "BuildAddStatus( '" + CommaSeparatedStatusNames + "');", true);
            }
            catch (Exception ex)
            { }
        ...
        protected void btnStatusSave_Click(object sender, EventArgs e)
        {
            ...
            string strSaveStatus = string.Empty; 
            DBResult objResult = new DBResult();
            try
            {
                UserBase loginUser = (UserBase)Session["LoggedUser"];
                ...
                objStatus.WorkflowStageStatusCurrentstatusId = Convert.ToInt32(hdnStatusId.Value);
                ...
                if (objResult.ErrorState == 0)
                {
                   ...
                   // clearStatusControls();
                    ...
                    if (action.Equals("AddStage"))
                    {
                       
                        hdnSaveFlagStatus.Value = "Yes";
                        //make button color green on sucessful save

                        ScriptManager.RegisterClientScriptBlock((sender as Control), this.GetType(), "ChangeColor", "ChangeAddStatusButtonColor('" + hdnSaveFlagStatus.Value + "');", true);
                    }
        ...
        protected void btnStageEdit_Click(object sender, EventArgs e)
        {
            ...
            {
                int Count = objResult.dsResult.Tables[1].Rows.Count;
                //Generateing workflow buttons based on process
                Dictionary<int, string> dictionary = new Dictionary<int, string>();
                foreach (DataRow row in objResult.dsResult.Tables[1].Rows)
                {
                    int buttonId = Convert.ToInt32(row["ID"]);
                    string buttonName = Convert.ToString(row["Name"]);
                    dictionary.Add(buttonId, buttonName);
               ...
               // ScriptManager.RegisterClientScriptBlock((sender as Control), this.GetType(), "BuildStageFields", "BuildStageFields( '" + values + "');", true);
            }
            ...
            {
                int Count = objResult.dsResult.Tables[2].Rows.Count;
                //Generateing workflow buttons based on process
                Dictionary<int, string> dictionary = new Dictionary<int, string>();
                foreach (DataRow row in objResult.dsResult.Tables[2].Rows)
                {
                    int buttonId = Convert.ToInt32(row["ID"]);
                    string buttonName = Convert.ToString(row["Name"]);
                    dictionary.Add(buttonId, buttonName);
        ...
        protected void btnStageSubmit_Click(object sender, EventArgs e)
        {
           // CheckAuthentication();
            ...
            try
            {
                ...
                if (objResult.ErrorState == 0)
                {
                    Session["StageId"] = Convert.ToInt32(objResult.dsResult.Tables[0].Rows[0]["StageId"]);
                    ...
                    if (action.Equals("AddStage"))
                    {
                       ...
                       // ScriptManager.RegisterClientScriptBlock((sender as Control), this.GetType(), "ChangeColor", "ChangeAddStageButtonColor('" + hdnSaveFlagStatus.Value + "' );", true);
                    }
        ...
        protected void btnStageAddOk_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string CommaSeparatedStageNames = string.Empty;
                foreach (GridViewRow row in gridStageMaster.Rows)
                {
                    ...
                    if (chk.Checked)
                    {
                        ...
                        CommaSeparatedStageNames += (row.Cells[2].Text + ",").Trim(); //StageName  
                    }
                }
               ...
               // BindStagesGridview(GetAllStages());
                ...
                CommaSeparatedStageNames = CommaSeparatedStageNames.TrimEnd(',');

                ScriptManager.RegisterClientScriptBlock((sender as Control), this.GetType(), "BuildAddStages", "BuildAddStages( '" + CommaSeparatedStageNames + "');", true);
            }
            catch (Exception ex)
            { }
        ...
        protected void btnNotificationSave_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (objResult.ErrorState == 0)
                {
                    ...
                    //hdnErrorStatus.Value = "";
                    if (action.Equals("AddNotificationConfiguration"))
                    {
                        hdnSaveFlagStatus.Value = "Yes";
                      
                    }
                    //make button color green on sucessful save
                    string Values = objNotification.NotificationCategory;
                    ScriptManager.RegisterClientScriptBlock((sender as Control), this.GetType(), "BuildAddNotification", "BuildAddNotification('" + Values + "');", true);
                }
                else
                {
                    if (strSaveStatus == "Save Changes")
        ...
        private void clearNotificationControls()
        {
            txt_TAT.Text = string.Empty;         
            chkNotificationActive.Checked = false;
        }
