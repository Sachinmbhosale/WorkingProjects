...
namespace Lotex.EnterpriseSolutions.WebUI.Secure.Core
{
    ...
    public partial class MultipleDocumentDownloadSearch : PageBase
    {

        //string[] bulkinv = Lotex.EnterpriseSolutions.CoreDAL.DocumentDAL.DocBulkDownload;
            ...
            using (ZipFile zip = new ZipFile())
            {
                try
                {
                    ...
                    foreach (GridViewRow gr in GridView1.Rows)
                    {
                        ...
                        if (chk.Checked)
                        {
                            listA.Add(gr.Cells[21].Text.ToString() + "~" + gr.Cells[7].Text.ToString());
                            string inv = gr.Cells[21].Text.ToString();
        ...
        public Results SearchDocumentsNew(SearchFilter filter, string action, string loginOrgId, string loginToken)
        {
            ...
            cmd.CommandText = "USP_SearchDocumentsNew";
            cmd.CommandType = CommandType.StoredProcedure;


            cmd.Parameters.AddWithValue("@in_iDocID", filter.UploadDocID);
            cmd.Parameters.AddWithValue("@in_iDocuTypeID", filter.DocumentTypeID);
            ...
            for (int i = 0; i < GridView1.Rows.Count; i++)
            {
                for (int j = 0; j < GridView1.Rows[i].Cells.Count; j++)
                {
                    ...
                    string encoded = GridView1.Rows[i].Cells[3].Text;
                    GridView1.Rows[i].Cells[3].Text = Context.Server.HtmlDecode(encoded);
        ...
        protected void btnSendMail(object sender, EventArgs e)
        {
            using (ZipFile zip = new ZipFile())
            {
                try
                {
                    ...
                    //var pth_extn_list = new List<pth_extn>();
                    for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                    {
                        filePath = ds.Tables[0].Rows[i]["UPLOAD_vDocPhysicalPath"].ToString();
                        filePath2 = beforedot(filePath) + ".zip" + "#" + System.IO.Path.GetExtension(filePath);
                        ...
                        //pth_extn = filePath2 + "#" + System.IO.Path.GetExtension(filePath);
                        //extn.Add(System.IO.Path.GetExtension(filePath));
                        //zip.AddFile(filePath2, Server.MapPath("/tmp") + "/DownloadedFile.zip");
                    ...
                    //Response.Redirect("Mailsendzip.ashx?path=" + sPath + "&detail=" + txtMailTo.Text + "~" + txtMessage.Text + "~" + txtsubject.Text);


                    using (MailMessage mm = new MailMessage("app_system@writercorporation.com", mailto))
                    {
                        mm.Subject = subject;
                        mm.Body = message;
                        ...
                        mm.Attachments.Add(new Attachment(sPath));
                        mm.IsBodyHtml = false;
                        SmtpClient smtp = new SmtpClient();
                        smtp.Host = "172.16.1.76";
                        smtp.EnableSsl = false;
                        mm.From = new MailAddress("dms@nokia.com");
                        NetworkCredential NetworkCred = new NetworkCredential("app_system@writercorporation.com", "sys1@345");
                        smtp.UseDefaultCredentials = true;
                        smtp.Credentials = NetworkCred;
                        smtp.Port = 25;
                        smtp.Send(mm);
                        ClientScript.RegisterStartupScript(GetType(), "alert", "alert('Email sent.');", true);
                        mailres.Text = "Mail sent successfully";
                    }
