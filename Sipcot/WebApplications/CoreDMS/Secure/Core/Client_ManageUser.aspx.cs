...
{
    public partial class Client_ManageUser : PageBase
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            btnUpdate.Attributes.Add("onClick", "return EncryptPassword1('" + txtNewpassword.Text + "');");
            ...
            if (!IsPostBack)
            {
                ...
                else
                {
                    ...
                    else if (action.ToLower() == "edit")
                    {
                        ...
                        // txtUserEmailId.Attributes.Add("readonly", "readonly");
                        txtDescription.Attributes.Add("readonly", "readonly");

                        txtUserName.BackColor = System.Drawing.ColorTranslator.FromHtml("#F2F0E1");
                        txtFirstName.BackColor = System.Drawing.ColorTranslator.FromHtml("#F2F0E1");
        ...
        protected void btnsearchagain_Click(object sender, EventArgs e)
        {
            ...
            Response.Redirect("Client_SearchUser.aspx?Search=" + SearchCriteria);
        }
        ...
        protected void btnUpdate_Click(object sender, EventArgs e)
        {
            ...
            if (txtNewpassword.Text != "" || txtConfirmpassword.Text != string.Empty && txtConfirmpassword.Text != "" || txtNewpassword.Text != string.Empty)
            {
                //if (txtNewpassword.Text == txtConfirmpassword.Text)
                //{
                //    string reg = @"^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[$@$!%*?&])[A-Za-z\d$@$!%*?&]{8,}";

                    //if (System.Text.RegularExpressions.Regex.IsMatch(txtNewpassword.Text, reg))
                    //{
                    //    int strcheck;
                    //    if (chkActive.Checked == true)
                    //    {
                    ...
                    //else
                    //{
                    //    ScriptManager.RegisterStartupScript(this.Page, this.Page.GetType(), "ValidationMessage", "alert(' Password should contain minimum 8 and maximum 20 characters, at least one special character and one numeric value.');", true);
                    //    return;
                ...
                //else
                //{
                //    ScriptManager.RegisterStartupScript(this.Page, this.Page.GetType(), "ValidationMessage", "alert('Password & Confirm password not match.');", true);
                //    return;
            ...
            string SelectedcDepartment = string.Empty;
            string SelectedDepartment = string.Empty;

            foreach (ListItem listItem in drpDepartment.Items)
            {
                if (listItem.Selected)
                {
                    var val = listItem.Value;
                    var txt = listItem.Text;
                    SelectedcDepartment = (SelectedcDepartment + val + ",");
                }
            }
            SelectedDepartment = SelectedcDepartment.Substring(0, SelectedcDepartment.Length - 1);

            //UserBase loginUser = (UserBase)Session["LoggedUser"];

    
            results1.ActionStatus = "SUCCESS";
            string connectionstring1 = ConfigurationManager.ConnectionStrings["SqlServerConnString"].ConnectionString.ToString();
            SqlConnection con1 = new SqlConnection(connectionstring1);
            con1.Open();
            SqlCommand cmd1 = new SqlCommand("USP_UpdateUsersDetails", con1);
            cmd1.CommandType = CommandType.StoredProcedure;
            SqlDataAdapter sda1 = new SqlDataAdapter(cmd1);
            cmd1.Parameters.Add(new SqlParameter("@in_iCurrUserId", Convert.ToInt32(hdnCurrentUserId.Value)));
            cmd1.Parameters.Add(new SqlParameter("@in_iOrgId", Convert.ToInt32(hdnLoginOrgId.Value)));
            cmd1.Parameters.Add(new SqlParameter("@vFirstname_Var", txtFirstName.Text.Trim()));
            cmd1.Parameters.Add(new SqlParameter("@vLastname_Var", txtLastName.Text.Trim()));
            cmd1.Parameters.Add(new SqlParameter("@vEmailId_Var", txtUserEmailId.Text.Trim()));
            cmd1.Parameters.Add(new SqlParameter("@vMobileNo_Var", txtMobileNo.Text.Trim()));
            cmd1.Parameters.Add(new SqlParameter("@vDepartmentIds_Var", SelectedDepartment.Trim()));
            cmd1.Parameters.Add(new SqlParameter("@iUserId_Var", Convert.ToInt32(loginUser.UserId)));

            cmd1.Parameters.Add(new SqlParameter("@SP_MESSAGE", SqlDbType.VarChar, 500)).Direction = ParameterDirection.Output;

            SqlDataAdapter da1 = new SqlDataAdapter(cmd1);
            DataSet ds1 = new DataSet();
            da1.Fill(ds1);
            results1.Message = cmd1.Parameters["@SP_MESSAGE"].Value.ToString();
            if (results1.Message == "Data updated successfully")
            {
                ScriptManager.RegisterStartupScript(this.Page, this.Page.GetType(), "ValidationMessage", "alert('Users details updated successfully.');", true);
                return;
            }
