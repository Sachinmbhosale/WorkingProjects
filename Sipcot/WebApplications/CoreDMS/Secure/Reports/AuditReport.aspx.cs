    ...
    public partial class AuditReport : PageBase
    {
                ...
                try
                {
                    ...
                    if (res.ActionStatus == "SUCCESS" && res.Users != null)
                    {
                        foreach (User user in res.Users)
                        {
                            ...
                            drpUser.Items.Add(new ListItem(user.UserName, user.UserId.ToString()));
        ...
        private void pBindData(string aSortExp, bool aIsCompleteData)
        {
            ...
            try
            {
                ...
                if (chkUser.Checked == true)
                {
                    ...
                    //else if(drpUser.Items[5].Selected == true)

                    else if (drpUser.Items.Count > 0)
                    {

                        var query = drpUser.Items.Cast<ListItem>().Where(item => item.Selected);
                        ...
                        int index = SelectedcDepartment.LastIndexOf(',');

                        SelectedcDepartment = SelectedcDepartment.Remove(index, 1);

                        action = "GetAuditDetailsForUserWithUserId";
                        //filter.CurrUserId = Convert.ToInt32(SelectedcDepartment);


                        filter.SelectedUserIdstring = Convert.ToString(SelectedcDepartment);




                        // SelectedcDepartment = SelectedcDepartment.Substring(0, SelectedcDepartment.Length - 1);


                        //action = "GetAuditDetailsForUserWithUserId";
                        //filter.CurrUserId = Convert.ToInt32(drpUser.SelectedValue);
                ...
                filter.Audittypes = Convert.ToString(cmbDocType.SelectedValue);
        ...
        protected void RdoPeriod_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (RdoPeriod.SelectedValue == "Last 1 week")
            {
                ...
                if (chkDoc.Checked == false && RdoPeriod.SelectedValue == "Last 1 week")
                {
            ...
            if (RdoPeriod.SelectedValue == "Last 1 month")
            {
                ...
                if (chkDoc.Checked == false && RdoPeriod.SelectedValue == "Last 1 month")
                {
            ...
            if (RdoPeriod.SelectedValue == "Last 6 months")
            {
                ...
                if (chkDoc.Checked == false && RdoPeriod.SelectedValue == "Last 6 months")
                {
